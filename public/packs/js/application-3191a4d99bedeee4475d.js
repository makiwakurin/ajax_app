  ...
  posts.forEach(function (post) {
    ...
    post.addEventListener("click", function () {
      ...
      XHR.onload = function () {
        ...
        var item = XHR.response.post; //既読状態であれば、灰色に変わるcssを適用する為のカスタムデータを追加している
...
function memo() {
  // 「投稿する」ボタンの情報を取得する
  var submit = document.getElementById("submit"); // 投稿するボタンを「click」した場合に実行される関数を定義

  submit.addEventListener("click", function (e) {
    // formDataはフォームに入力された値を取得できるオブジェクトのことです。
    var formData = new FormData(document.getElementById("form")); //XMLHttpRequestのオブジェクトを生成

    var XHR = new XMLHttpRequest(); //HTTPメソッドはPOST、パスは/posts、非同期通信はtrue
    ...
    XHR.onload = function () {
      ...
      var item = XHR.response.post; //listはHTMLを描画する場所を指定する「描画する親要素」
      //のlistの要素を取得しています。

      var list = document.getElementById("list"); //formTextを取得する理由は、メモの入力フォームをリセットするためです。この処理が終了した時に、
      // 入力フォームの文字は入力されたままになってしまうため、
      // リセットする必要があります。ここではリセット対象の要素であるcontentという要素を取得しています。

      var formText = document.getElementById("content"); //[メモとして描画する部分のHTML]を定義してる。
      //HTMLと言う変数を描画する様な処理を行えば、ここで定義したHTMLが
      // 描画される

      var HTML = "\n        <div class=\"post\" data-id=".concat(item.id, ">\n          <div class=\"post-date\">\n            \u6295\u7A3F\u65E5\u6642\uFF1A").concat(item.created_at, "\n          </div>\n          <div class=\"post-content\">\n          ").concat(item.content, "\n          </div>\n        </div>"); //listと言う要素に対して、insertAdjacentHTMLでHTML
      //を追加します。
...
/******/ });
//# sourceMappingURL=application-3191a4d99bedeee4475d.js.map